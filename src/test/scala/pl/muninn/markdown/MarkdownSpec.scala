...
import pl.muninn.markdown.common.basic.block.Table.{ColumnAlignment, col, header, headers, row, setDefaultAlignment}

class MarkdownSpec extends munit.FunSuite:
  test("should allow and render every possible component") {
    md {
      h("test")
      h1("test")
      h2("test")
      ...
      table {
        setDefaultAlignment(ColumnAlignment.Center)
        headers {
          header(ColumnAlignment.Left) {
            "test"
          }
        }
        row {
      ...
      tableAligned(ColumnAlignment.Center) {
        headers {
          header(ColumnAlignment.Left) {
            "test"
          }
  ...
  test("should allow and render every possible component using partials") {
    ...
    val s           = partial.s("test")
    val taskList = partial.tasks {
      task {
        "test"
      }
    }
    val table = partial.table {
      headers {
        col("test")
      }
      row {
